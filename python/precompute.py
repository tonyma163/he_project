...
trained_weight = torch.load('SST-2-BERT-tiny.bin')
...
for layer, input_list in layernorm_inputs.items():
    ...
    mean = all_inputs.mean(dim=0)
    var = all_inputs.var(dim=0, unbiased=False)
    ...
    np.savetxt(f"{layer}_mean.txt", mean.numpy())
    np.savetxt(f"{layer}_inv_sqrt_var.txt", inv_sqrt_var.numpy())
